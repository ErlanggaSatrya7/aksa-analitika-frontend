...
export default function CityProfilePage() {
    ...
    const handleSave = () => {
        setToast({ message: "Profil Manager Kota berhasil diperbarui", type: 'success' });
    ...
    return (
        <div className="pb-10 max-w-6xl mx-auto animate-in fade-in duration-500 relative">
            ...
            <div className="mb-6 md:mb-8">
                ...
                <p className="text-xs md:text-sm text-slate-500 mt-1">Kelola profil manajerial dan pengaturan keamanan akun Kota Anda.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-6 md:gap-8">
                ...
                <div className="flex-1 bg-white border border-slate-100 rounded-[32px] md:rounded-[40px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] p-6 md:p-8 lg:p-10 min-h-[500px]">
                    ...
                    {activeTab === 'profil' && (
                        <div className="animate-in fade-in slide-in-from-right-4">
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8 md:mb-10 pb-8 md:pb-10 border-b border-slate-100">
                                <img src="https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=100&h=100&fit=crop" alt="Citra Profile" className="w-20 h-20 md:w-24 md:h-24 rounded-full object-cover shadow-md border-4 border-[#EDF2FE]" />
                                <div className="flex flex-wrap gap-2 sm:gap-3">
                                    {/* UPDATE: Tombol Ganti Foto Premium */}
                                    <button className="text-xs md:text-sm font-bold text-white bg-gradient-to-r from-[#6A7BFA] to-[#4f46e5] hover:shadow-lg px-5 md:px-6 py-2.5 md:py-3 rounded-full transition-all shadow-md active:scale-95">Ganti Foto</button>
                                    <button className="text-xs md:text-sm font-bold text-slate-500 bg-white hover:bg-red-50 hover:text-red-500 hover:border-red-200 px-5 md:px-6 py-2.5 md:py-3 rounded-full transition-colors border border-slate-200">Hapus</button>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl">
                                <div>
                                    ...
                                    <input type="text" defaultValue="Citra Lestari" className="w-full mt-2 bg-slate-50 border border-slate-200 rounded-[20px] px-5 py-3.5 md:py-4 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#4f46e5]/20 focus:border-[#4f46e5] transition-all" />
                                </div>
                                <div>
                                    <label className="text-[11px] md:text-xs font-bold text-slate-500 uppercase tracking-widest ml-2">Email Perusahaan</label>
                                    <input type="email" defaultValue="citra.l@aksa.com" disabled className="w-full mt-2 bg-slate-100 border border-slate-200 rounded-[20px] px-5 py-3.5 md:py-4 text-sm font-semibold text-slate-400 cursor-not-allowed" />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="text-[11px] md:text-xs font-bold text-slate-500 uppercase tracking-widest ml-2">Area Otorisasi (City)</label>
                                    <input type="text" defaultValue="Manager Kota Medan (Metropolitan)" disabled className="w-full mt-2 bg-slate-100 border border-slate-200 rounded-[20px] px-5 py-3.5 md:py-4 text-sm font-semibold text-slate-400 cursor-not-allowed" />
                                </div>
                            </div>
                        </div>
                    )}
                            ...
                            <div className="space-y-4">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 md:p-6 bg-slate-50 border border-slate-200 rounded-3xl gap-4">
                                    <div className="flex gap-4 items-center">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm md:text-base text-slate-900">Alert Toko Kritis</p>
                                            <p className="text-xs md:text-sm text-slate-500 mt-1">Kirim peringatan ke email jika stok toko di Medan menipis secara drastis.</p>
                                        </div>
                                    </div>
                                    {/* UPDATE: Toggle Switch Premium */}
                                    <div className="w-14 h-8 bg-gradient-to-r from-[#6A7BFA] to-[#4f46e5] rounded-full relative cursor-pointer shadow-inner shrink-0">
                        ...
                        <div className="animate-in fade-in slide-in-from-right-4 max-w-3xl">
                            ...
                            <div className="space-y-5 md:space-y-6">
                                <div>
                                    ...
                                    <input type="password" placeholder="••••••••" className="w-full mt-2 bg-slate-50 border border-slate-200 rounded-[20px] px-5 py-3.5 md:py-4 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#4f46e5]/20 focus:border-[#4f46e5] transition-all" />
                                </div>
                                <div>
                                    <label className="text-[11px] md:text-xs font-bold text-slate-500 uppercase tracking-widest ml-2">Kata Sandi Baru</label>
                                    <input type="password" placeholder="Minimal 8 karakter" className="w-full mt-2 bg-slate-50 border border-slate-200 rounded-[20px] px-5 py-3.5 md:py-4 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#4f46e5]/20 focus:border-[#4f46e5] transition-all" />
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="mt-8 md:mt-10 pt-6 md:pt-8 border-t border-slate-100 flex justify-end">
                        {/* UPDATE: Tombol Simpan Gradient */}
                        <button onClick={handleSave} className="w-full md:w-auto px-10 py-4 bg-gradient-to-r from-[#6A7BFA] to-[#4f46e5] text-white font-bold text-sm rounded-[40px] shadow-lg shadow-[#4f46e5]/20 hover:shadow-[#4f46e5]/40 hover:-translate-y-0.5 transition-all active:scale-95">
                            Simpan Perubahan
                        </button>
                    </div>
