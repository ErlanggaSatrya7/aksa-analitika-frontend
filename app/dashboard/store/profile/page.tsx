...
import { User, BellRing, ShieldCheck, AlertTriangle, CheckCircle2, TrendingUp } from 'lucide-react';
...
export default function StoreProfilePage() {
    ...
    const handleSave = () => {
        setToast({ message: "Profil Supervisor berhasil diperbarui", type: 'success' });
    ...
    return (
        <div className="pb-10 max-w-6xl mx-auto animate-in fade-in duration-500 relative">
            ...
            <div className="mb-6 md:mb-8">
                ...
                <p className="text-xs md:text-sm text-slate-500 mt-1">Kelola profil operasional dan pengaturan keamanan cabang Anda.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-6 md:gap-8">
                ...
                <div className="flex-1 bg-white border border-slate-100 rounded-[32px] md:rounded-[40px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] p-6 md:p-8 lg:p-10 min-h-[500px]">
                    ...
                    {activeTab === 'profil' && (
                        <div className="animate-in fade-in slide-in-from-right-4">
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8 md:mb-10 pb-8 md:pb-10 border-b border-slate-100">
                                <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop" alt="Profile" className="w-20 h-20 md:w-24 md:h-24 rounded-full object-cover shadow-md border-4 border-[#EDF2FE]" />
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl">
                                <div>
                                    ...
                                    <input type="text" defaultValue="Eka Saputra" className="w-full mt-2 bg-slate-50 border border-slate-200 rounded-[20px] px-5 py-3.5 md:py-4 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#6A7BFA]/20 focus:border-[#6A7BFA]" />
                                </div>
                                <div>
                                    <label className="text-[11px] md:text-xs font-bold text-slate-500 uppercase tracking-widest ml-2">ID Cabang (Kode Toko)</label>
                                    <input type="text" defaultValue="STR-MDN-001" disabled className="w-full mt-2 bg-slate-100 border border-slate-200 rounded-[20px] px-5 py-3.5 md:py-4 text-sm font-semibold text-slate-400 cursor-not-allowed" />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="text-[11px] md:text-xs font-bold text-slate-500 uppercase tracking-widest ml-2">Lokasi Penugasan</label>
                                    <input type="text" defaultValue="Supervisor Ramayana Plaza Medan" disabled className="w-full mt-2 bg-slate-100 border border-slate-200 rounded-[20px] px-5 py-3.5 md:py-4 text-sm font-semibold text-slate-400 cursor-not-allowed" />
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="mt-8 md:mt-10 pt-6 md:pt-8 border-t border-slate-100 flex justify-end">
                        <button onClick={handleSave} className="w-full md:w-auto px-10 py-4 bg-[#6A7BFA] text-white font-bold text-sm rounded-[40px] shadow-xl shadow-[#6A7BFA]/20 hover:bg-[#5869E8] hover:shadow-[#6A7BFA]/40 transition-all active:scale-95">
                            Simpan Perubahan
                        </button>
                    </div>
