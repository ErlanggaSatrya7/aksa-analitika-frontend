...
export default function StoreSalesAnalysis() {
    ...
    const storeName = user?.retailerName || 'Cabang Anda';
    ...
    // State untuk menampung data dinamis dari DB
    ...
    // Fetch API Penjualan Toko berdasarkan Kategori
    useEffect(() => {
        ...
        const fetchSales = async () => {
            try {
                ...
                // Tambahkan pewarnaan pada Pie Chart
                const colors = ['#6A7BFA', '#312E81', '#A3B1FF', '#4f46e5'];
                const coloredPie = data.pieChart.map((item: any, i: number) => ({
                    ...item,
                    itemStyle: { color: colors[i % colors.length] }
                }));

                setSalesData({ ...data, pieChart: coloredPie });
            } catch (error) {
    ...
    const formatRupiah = (val: number) => {
        if (val >= 1_000_000) return `Rp ${(val / 1_000_000).toFixed(1)}k`;
    ...
    const dailyTrendOption = {
        ...
        yAxis: { type: 'value', splitLine: { lineStyle: { type: 'dashed', color: '#F1F5F9' } }, axisLabel: { formatter: '{value}', color: '#64748B' } },
        series: [{
    ...
    // 2. CHART: Metode Penjualan / Pembayaran
    const paymentOption = {
        ...
        series: [{
            name: 'Metode Transaksi', type: 'pie', radius: ['45%', '70%'], center: ['50%', '40%'], avoidLabelOverlap: false,
    ...
    if (isLoading) return <div className="w-full h-full flex flex-col items-center justify-center text-[#6A7BFA] gap-3 min-h-[60vh]"><Loader2 size={32} className="animate-spin" /><span className="font-bold tracking-widest uppercase text-sm">Menyiapkan Analitik...</span></div>;
        ...
        <div className="pb-10 max-w-7xl mx-auto space-y-6 relative">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1 flex items-center gap-1.5"><Activity size={16} className="text-[#6A7BFA]" /> Pantau tren harian dan perilaku pembeli di {storeName}.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-6 duration-700 delay-200 relative z-10">
                <div className="bg-white rounded-[32px] p-6 lg:p-8 border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.03)] flex flex-col justify-between hover:shadow-[0_20px_40px_-15px_rgba(106,123,250,0.2)] hover:-translate-y-1.5 transition-all duration-300 group">
                    ...
                    <div>
                        <p className="text-[11px] lg:text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5 group-hover:text-[#6A7BFA] transition-colors">Volume Unit Terjual</p>
                        <h3 className="text-3xl lg:text-4xl font-bold text-slate-900 tracking-tight">{salesData.kpi.volume.toLocaleString('id-ID')} <span className="text-xl lg:text-2xl text-slate-400 font-medium tracking-normal">Pcs</span></h3>
                    </div>
                </div>
                <div className="bg-white rounded-[32px] p-6 lg:p-8 border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.03)] flex flex-col justify-between hover:shadow-[0_20px_40px_-15px_rgba(106,123,250,0.2)] hover:-translate-y-1.5 transition-all duration-300 group">
                    ...
                    <div>
                        <p className="text-[11px] lg:text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5 group-hover:text-[#6A7BFA] transition-colors">Avg. Belanja / Transaksi</p>
                        <h3 className="text-3xl lg:text-4xl font-bold text-slate-900 tracking-tight">{formatRupiah(salesData.kpi.avgTicket)}</h3>
                    </div>
                </div>
                <div className="bg-white rounded-[32px] p-6 lg:p-8 border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.03)] flex flex-col justify-between hover:shadow-[0_20px_40px_-15px_rgba(106,123,250,0.2)] hover:-translate-y-1.5 transition-all duration-300 group">
                    ...
                    <div>
                        <p className="text-[11px] lg:text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5 group-hover:text-[#6A7BFA] transition-colors">Hari Transaksi Tertinggi</p>
                        <h3 className="text-3xl lg:text-4xl font-bold text-slate-900 tracking-tight">{salesData.kpi.topDay}</h3>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-6 duration-700 delay-300 relative z-10">
                <div className="lg:col-span-2 bg-white border border-slate-100 rounded-[40px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] p-6 md:p-8 flex flex-col group">
                    ...
                    <p className="text-sm text-slate-500 mb-6">Distribusi volume penjualan di {storeName} per hari.</p>
                    <div className="w-full h-[350px]">
                        {salesData.dailyTrend.some(v => v > 0) ? (
                ...
                <div className="bg-white border border-slate-100 rounded-[40px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] p-6 md:p-8 flex flex-col group">
                    ...
                    <p className="text-sm text-slate-500 mb-6">Preferensi transaksi pelanggan {filterCategory !== 'Semua Kategori' ? `untuk ${filterCategory}` : 'secara umum'}.</p>
                    <div className="w-full h-[350px] group-hover:scale-[1.02] transition-transform duration-500">
                        {salesData.pieChart.length > 0 ? (
                            <ReactECharts option={paymentOption} style={{ height: '100%', width: '100%' }} opts={{ renderer: 'svg' }} />
                        ) : (
                            <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 gap-2"><AlertTriangle className="opacity-50" size={32} /></div>
                        )}
                    </div>
                </div>
            </div>
