...
import { Search, Plus, Filter, MoreVertical, Edit, ShieldAlert, Mail, MapPin, ShieldCheck, UserCog, Building2, Store, Lock, CheckCircle2, X, Loader2, ChevronDown } from 'lucide-react';
...
export default function KelolaPenggunaPage() {
    ...
    // STATE MASTER WILAYAH DINAMIS (Dari Supabase)
    ...
    // STATE FORM & MODAL INVITE
    const [isInviteModalOpen, setIsInviteModalOpen] = useState(false);
    const [isLoading, setIsLoading] = useState(false);
    const [formData, setFormData] = useState({
        fullName: '', email: '', role: 'CITY_ADMIN', state: '', city: '', retailer: ''
    });

    // STATE MODAL CRUD (DETAIL, EDIT, SUSPEND)
    const [selectedUser, setSelectedUser] = useState<DBUser | null>(null);
    const [isDetailModalOpen, setIsDetailModalOpen] = useState(false);
    const [isEditModalOpen, setIsEditModalOpen] = useState(false);
    const [isSuspendModalOpen, setIsSuspendModalOpen] = useState(false);
    const [editFormData, setEditFormData] = useState({ role: '', state: '', city: '', retailer: '' });
    ...
    // AMBIL DATA DARI SUPABASE (User & Lokasi)
    ...
    // MENGGUNAKAN DATA RIIL DATABASE UNTUK MENAMPILKAN WILAYAH AKSES DI TABEL
    const getRoleComponents = (user: DBUser) => {
        ...
        if (user.role === 'RETAILER_ADMIN') {
            ...
            return { badge: <span className="flex items-center gap-1.5 w-fit px-2.5 py-1 bg-amber-100 text-amber-700 text-[11px] font-bold rounded-md"><Store size={12} /> Manajer Retailer</span>, scope: foundRetailer ? foundRetailer.name : 'Mitra Toko Belum Diatur' };
        }
    ...
    const handleUpdateAccess = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                setToast({ type: 'success', message: 'Hak akses pengguna berhasil diperbarui.' });
                loadAllData(); setTimeout(() => setIsEditModalOpen(false), 2000);
            } else setToast({ type: 'error', message: 'Gagal memperbarui hak akses.' });
        } catch (error) { setToast({ type: 'error', message: 'Terjadi kesalahan sistem.' }); } finally { setIsLoading(false); }
    };
    ...
    const openEditModal = (user: DBUser) => {
        ...
        setEditFormData({ role: user.role, state: user.assignedState || '', city: user.assignedCity || '', retailer: user.retailerId || '' });
    ...
    return (
        <div className="pb-10 max-w-7xl mx-auto space-y-6 relative">
                                        ...
                                        <td className="px-6 py-4 relative text-center">
                                            ...
                                            {openDropdownId === user.id && (
                                                <div ref={dropdownRef} className="absolute right-10 top-1/2 -translate-y-1/2 w-48 bg-white border border-slate-100 rounded-[16px] shadow-[0_20px_50px_-15px_rgba(0,0,0,0.15)] z-[60] p-1.5 animate-in fade-in zoom-in-95">
                                                    ...
                                                    <button onClick={() => openEditModal(user)} className="w-full flex items-center gap-2.5 px-3 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-50 hover:text-amber-600 rounded-[10px] transition-colors"><Edit size={14} /> Ubah Akses</button>
                                                    <div className="h-px bg-slate-100 my-1"></div>
                                                    <button onClick={() => { setSelectedUser(user); setIsSuspendModalOpen(true); setOpenDropdownId(null); }} className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-xs font-bold rounded-[10px] transition-colors ${user.status === 'ACTIVE' ? 'text-red-500 hover:bg-red-50' : 'text-emerald-600 hover:bg-emerald-50'}`}>
            ...
            {isSuspendModalOpen && selectedUser && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[150] flex items-center justify-center p-4 animate-in fade-in duration-300">
                    <div className="bg-white rounded-[32px] w-full max-w-sm p-8 shadow-2xl relative border border-slate-100 text-center animate-in zoom-in-95">
                        <div className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-5 ${selectedUser.status === 'ACTIVE' ? 'bg-red-50 text-red-500' : 'bg-emerald-50 text-emerald-500'}`}>
                        ...
                        {toast && <div className={`mb-4 text-sm font-bold text-center p-3 rounded-xl border ${toast.type === 'success' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-red-50 text-red-700 border-red-100'}`}>{toast.message}</div>}
                        <div className="flex gap-3">
                            <button onClick={() => setIsSuspendModalOpen(false)} className="flex-1 px-4 py-3 bg-slate-50 text-slate-600 font-bold text-sm rounded-[20px] hover:bg-slate-100 transition-colors">Batal</button>
                            <button onClick={handleToggleStatus} disabled={isLoading} className={`flex-1 px-4 py-3 text-white font-bold text-sm rounded-[20px] shadow-lg transition-all flex items-center justify-center gap-2 ${selectedUser.status === 'ACTIVE' ? 'bg-red-500 hover:bg-red-600 shadow-red-500/30' : 'bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/30'}`}>
            ...
            {isEditModalOpen && selectedUser && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[150] flex items-center justify-center p-4 animate-in fade-in duration-300">
                    <div className="bg-white rounded-[32px] w-full max-w-lg p-8 shadow-2xl animate-in zoom-in-95 flex flex-col relative border border-slate-100 max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="mb-6"><h3 className="text-2xl font-bold text-slate-900 tracking-tight">Ubah Akses Wilayah</h3><p className="text-sm text-slate-500 mt-1">Ubah otorisasi untuk <span className="font-bold">{selectedUser.fullName}</span>.</p></div>
