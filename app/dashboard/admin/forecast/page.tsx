...
import { BrainCircuit, AlertCircle, Loader2, Info, MapPin, Building2, Store, PackageSearch, Sparkles, ChevronDown, Lightbulb, Clock, BellRing, TrendingDown, ShieldCheck, History, CheckCircle2, ChevronUp, Send } from 'lucide-react';
...
export default function AIForecastingPage() {
    ...
    // State Parameter Filter (Nasional)
    ...
    // Insight Level Nasional
    const [currentInsightText, setCurrentInsightText] = useState('AI mendeteksi anomali Nasional: Potensi defisit pasokan di Pulau Jawa dan Sumatera bulan depan pada skenario pesimis. Segera instruksikan Manajer Provinsi untuk penyesuaian anggaran logistik.');

    const [isHistoryOpen, setIsHistoryOpen] = useState(false);

    // History Broadcast Nasional (Top-Down ke Provinsi)
    const [broadcastHistory, setBroadcastHistory] = useState([
        { id: 'BRD-NAT-092', date: 'Senin, 11 Mei 2026 08:30 WIB', target: 'Manajer Provinsi (Jawa Timur)', insight: 'Persetujuan injeksi anggaran logistik sebesar Rp 2M untuk menutupi defisit Kids Apparel.' },
        { id: 'BRD-NAT-091', date: 'Jumat, 8 Mei 2026 14:15 WIB', target: 'Seluruh Manajer Provinsi', insight: 'Memo Global: Persiapan distribusi stok massal menyambut kampanye Mega Sale Akhir Tahun.' }
    ]);
    ...
    const handleGenerateForecast = () => {
        ...
        setTimeout(() => {
            ...
            setCurrentInsightText(`Proyeksi Makro: Komparasi AI dengan API Logistik Nasional mendeteksi lonjakan permintaan ${produk} untuk ${retailerText} di ${area}. Sistem merekomendasikan rilis kuota cadangan ke gudang provinsi terkait.`);
            showToast("Analisis Selesai", "Prediksi AI tingkat nasional berhasil dimuat.");
        }, 1500);
    };
    ...
    const handleBroadcastAlert = () => {
        ...
        setTimeout(() => {
            ...
            const newLog = { id: `BRD-NAT-${Math.floor(Math.random() * 900) + 100}`, date: updateTimestamp(), target: targetCabang, insight: currentInsightText };
    ...
    const forecastOption = {
        ...
        legend: { type: 'scroll', data: ['Data Aktual', 'Skenario Optimis', 'Prediksi Utama', 'Skenario Pesimis'], bottom: 0, icon: 'circle', textStyle: { color: '#475569', fontSize: 11, fontWeight: '600' }, itemGap: 15 },
        ...
        series: [
            // UPDATE: Warna Data Aktual diselaraskan dengan tema gradasi Premium (Lebih ke 4f46e5)
            { name: 'Data Aktual', type: 'line', smooth: true, symbolSize: 8, itemStyle: { color: '#4f46e5', borderWidth: 2, borderColor: '#fff' }, lineStyle: { width: 4, color: '#4f46e5' }, areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(106, 123, 250, 0.4)' }, { offset: 1, color: 'rgba(79, 70, 229, 0)' }] } }, data: [1.2, 1.5, 1.3, 1.8, 2.1, null, null, null] },
            { name: 'Skenario Optimis', type: 'line', smooth: true, symbolSize: 6, itemStyle: { color: '#10B981', borderWidth: 2, borderColor: '#fff' }, lineStyle: { width: 2, type: 'dashed', color: '#10B981' }, data: [null, null, null, null, 2.1, 2.6, 2.9, 3.3] },
            {
                name: 'Prediksi Utama', type: 'line', smooth: true, symbolSize: 8, itemStyle: { color: '#F59E0B', borderWidth: 2, borderColor: '#fff' }, lineStyle: { width: 3, type: 'dashed', color: '#F59E0B' }, data: [null, null, null, null, 2.1, 2.4, 2.6, 2.9],
                markLine: { symbol: 'none', label: { formatter: 'Titik Prediksi', position: 'end', color: '#64748B', fontSize: 10, fontWeight: '600', padding: [0, 0, 5, 0] }, lineStyle: { color: '#CBD5E1', type: 'dashed', width: 1.5 }, data: [{ xAxis: 'Mei' }] }
            },
            { name: 'Skenario Pesimis', type: 'line', smooth: true, symbolSize: 6, itemStyle: { color: '#EF4444', borderWidth: 2, borderColor: '#fff' }, lineStyle: { width: 2, type: 'dashed', color: '#EF4444' }, data: [null, null, null, null, 2.1, 2.2, 2.3, 2.5] }
        ]
    };

    if (isLoading) return <div className="w-full h-full flex items-center justify-center text-[#6A7BFA] gap-3"><Loader2 size={24} className="animate-spin" /><span className="font-bold tracking-widest uppercase text-sm">Memuat Engine AI...</span></div>;
                ...
                <div className="flex flex-col xl:flex-row xl:items-end gap-6">
                    ...
                    <div ref={filterRef} className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className={`flex flex-col gap-1.5 relative transition-all ${openDropdown === 'prov' ? 'z-50' : 'z-10'}`}>
                            ...
                            <button
                                onClick={() => setOpenDropdown(openDropdown === 'prov' ? null : 'prov')}
                                className={`flex items-center justify-between bg-white border ${openDropdown === 'prov' ? 'border-[#4f46e5] ring-4 ring-[#6A7BFA]/10 shadow-sm' : 'border-slate-200 hover:border-[#6A7BFA]/50 hover:shadow-md hover:-translate-y-0.5'} rounded-[20px] px-4 py-3 text-sm font-bold text-slate-700 transition-all duration-300 group`}
                            >
                            ...
                            {openDropdown === 'prov' && (
                                <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-white border border-slate-100 rounded-[20px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] z-[60] p-2 animate-in fade-in slide-in-from-top-2">
                                    {['Semua Provinsi', 'DKI Jakarta', 'Jawa Timur', 'Jawa Barat', 'Sumatera Utara'].map((item) => (
                                        <button
                                            key={item}
                                            onClick={() => { setFilterProv(item); setOpenDropdown(null); }}
                                        ...
                                        >
                                            {item}
                                            {filterProv === item && <CheckCircle2 size={16} className="text-white" />}
                                        </button>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className={`flex flex-col gap-1.5 relative transition-all ${openDropdown === 'city' ? 'z-50' : 'z-10'}`}>
                            ...
                            <button
                                ...
                                className={`flex items-center justify-between bg-white border ${openDropdown === 'city' ? 'border-[#4f46e5] ring-4 ring-[#6A7BFA]/10 shadow-sm' : 'border-slate-200 hover:border-[#6A7BFA]/50 hover:shadow-md hover:-translate-y-0.5'} rounded-[20px] px-4 py-3 text-sm font-bold text-slate-700 transition-all duration-300 group`}
                            >
                                <div className="flex items-center gap-2 truncate">
                            ...
                            {openDropdown === 'city' && (
                                <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-white border border-slate-100 rounded-[20px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] z-[60] p-2 animate-in fade-in slide-in-from-top-2">
                                    {['Semua Kota', 'Jakarta Selatan', 'Surabaya', 'Medan', 'Bandung'].map((item) => (
                                        <button
                                            key={item}
                                            onClick={() => { setFilterCity(item); setOpenDropdown(null); }}
                                            className={`w-full flex items-center justify-between px-4 py-2.5 rounded-[12px] text-sm font-bold transition-all duration-200 ${filterCity === item ? 'bg-gradient-to-r from-[#6A7BFA] to-[#4f46e5] text-white' : 'text-slate-600 hover:bg-[#F4F7FE] hover:text-[#4f46e5] hover:translate-x-1'}`}
                                        >
                                            {item}
                                            {filterCity === item && <CheckCircle2 size={16} className="text-white" />}
                                        </button>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className={`flex flex-col gap-1.5 relative transition-all ${openDropdown === 'retailer' ? 'z-50' : 'z-10'}`}>
                            ...
                            <button
                                onClick={() => setOpenDropdown(openDropdown === 'retailer' ? null : 'retailer')}
                                className={`flex items-center justify-between bg-white border ${openDropdown === 'retailer' ? 'border-[#4f46e5] ring-4 ring-[#6A7BFA]/10 shadow-sm' : 'border-slate-200 hover:border-[#6A7BFA]/50 hover:shadow-md hover:-translate-y-0.5'} rounded-[20px] px-4 py-3 text-sm font-bold text-slate-700 transition-all duration-300 group`}
                            >
                                <div className="flex items-center gap-2 truncate">
                            ...
                            {openDropdown === 'retailer' && (
                                <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-white border border-slate-100 rounded-[20px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] z-[60] p-2 animate-in fade-in slide-in-from-top-2">
                                    {['Semua Retailer', 'Ramayana', 'Matahari', 'Sport Station'].map((item) => (
                                        <button
                                            key={item}
                                            onClick={() => { setFilterRetailer(item); setOpenDropdown(null); }}
                                            className={`w-full flex items-center justify-between px-4 py-2.5 rounded-[12px] text-sm font-bold transition-all duration-200 ${filterRetailer === item ? 'bg-gradient-to-r from-[#6A7BFA] to-[#4f46e5] text-white' : 'text-slate-600 hover:bg-[#F4F7FE] hover:text-[#4f46e5] hover:translate-x-1'}`}
                                        >
                                            {item}
                                            {filterRetailer === item && <CheckCircle2 size={16} className="text-white" />}
                                        </button>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className={`flex flex-col gap-1.5 relative transition-all ${openDropdown === 'product' ? 'z-50' : 'z-10'}`}>
                            ...
                            <button
                                ...
                                className={`flex items-center justify-between bg-white border ${openDropdown === 'product' ? 'border-[#4f46e5] ring-4 ring-[#6A7BFA]/10 shadow-sm' : 'border-slate-200 hover:border-[#6A7BFA]/50 hover:shadow-md hover:-translate-y-0.5'} rounded-[20px] px-4 py-3 text-sm font-bold text-slate-700 transition-all duration-300 group`}
                            >
                                <div className="flex items-center gap-2 truncate">
                            ...
                            {openDropdown === 'product' && (
                                <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-white border border-slate-100 rounded-[20px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] z-[60] p-2 animate-in fade-in slide-in-from-top-2">
                                    {['Semua Kategori Produk', "Men's Athletic", "Women's Street", "Kids Apparel"].map((item) => (
                                        <button
                                            key={item}
                                            onClick={() => { setFilterProduct(item); setOpenDropdown(null); }}
                                            className={`w-full flex items-center justify-between px-4 py-2.5 rounded-[12px] text-sm font-bold transition-all duration-200 ${filterProduct === item ? 'bg-gradient-to-r from-[#6A7BFA] to-[#4f46e5] text-white' : 'text-slate-600 hover:bg-[#F4F7FE] hover:text-[#4f46e5] hover:translate-x-1'}`}
                                        >
                                            {item}
                                            {filterProduct === item && <CheckCircle2 size={16} className="text-white" />}
                                        </button>
                                    ))}
                                </div>
                            )}
                    ...
                    <div className="w-full xl:w-auto shrink-0 relative z-20">
                        {/* UPDATE: Tombol Generate menggunakan Gradasi Premium */}
                        <button onClick={handleGenerateForecast} disabled={isPredicting || isBroadcasting} className="w-full xl:w-auto bg-gradient-to-r from-[#6A7BFA] to-[#4f46e5] hover:shadow-lg hover:shadow-[#4f46e5]/30 text-white px-8 py-3.5 rounded-[20px] font-bold text-sm transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-70">
                            {isPredicting ? <><Loader2 size={18} className="animate-spin" /> Menarik Data...</> : <><Sparkles size={18} /> Komparasi Data</>}
                        </button>
                    </div>
                </div>
            </div>

            {/* CHART CONTAINER & ACTIONABLE INSIGHT */}
            <div className="bg-white border border-slate-100 rounded-[40px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] p-6 md:p-8 flex flex-col relative overflow-hidden animate-in fade-in slide-in-from-bottom-6 duration-700 ease-out delay-200 fill-mode-both z-10">
                {isPredicting && (
                    <div className="absolute inset-0 bg-white/90 backdrop-blur-md z-20 flex flex-col items-center justify-center text-[#4f46e5] gap-4 animate-in fade-in duration-300">
                        ...
                        <span className="font-bold text-xl text-slate-800">Menyusun Ulang Prediksi...</span>
                    </div>
                )}
                ...
                {!isPredicting && (
                    <div className="mt-8 p-6 lg:p-8 border bg-[#F8FAFC] border-slate-200 rounded-[32px] shadow-inner relative overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="flex justify-end relative z-10 border-t border-slate-200 pt-5">
                            {/* UPDATE: Tombol Broadcast menggunakan Gradasi Premium */}
                            <button
