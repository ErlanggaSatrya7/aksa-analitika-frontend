...
export default function PetaDistribusiPage() {
    ...
    useEffect(() => {
        // Fetch GeoJSON dan Initial Data dari DB
        Promise.all([
            fetch('/indonesia.json').then((res) => res.json()),
            fetch(`/api/admin/analytics/map?province=Nasional`).then((res) => res.json())
        ]).then(([geojson, dbData]) => {
            // Translator Nama GeoJSON
            const geojsonNameTranslator: Record<string, string> = { "IRIAN JAYA TIMUR": "Papua", "IRIAN JAYA TENGAH": "Papua", "IRIAN JAYA BARAT": "Papua", "NUSATENGGARA BARAT": "NTB", "NUSA TENGGARA TIMUR": "NTT", "DAERAH ISTIMEWA YOGYAKARTA": "DI Yogyakarta", "PROBANTEN": "Banten", "DI. ACEH": "NAD Aceh" };

            geojson.features.forEach((feature: any) => {
                const oldName = feature.properties.Propinsi;
                if (geojsonNameTranslator[oldName]) feature.properties.Propinsi = geojsonNameTranslator[oldName];
            });

            echarts.registerMap('indonesia_interaktif', geojson);
            setMapAnalytics(dbData);
            setAllProvinces(dbData.availableProvinces || []);
            setIsLoading(false);
        }).catch(() => setIsLoading(false));
    }, []);

    const filteredProvinces = allProvinces.filter(prov => prov.toLowerCase().includes(searchProv.toLowerCase()));

    // Fungsi Trigger Refresh Data ke API Database
    ...
    const mainMapOption = {
        ...
        tooltip: { trigger: 'item', formatter: (params: any) => `<div style="font-weight:600; font-size:13px; color:#0F172A; text-transform:capitalize;">${params.name}</div><div style="color:#6A7BFA; font-weight:bold; margin-top:4px;">Volume: ${params.value || 0}</div>`, backgroundColor: '#ffffff', borderWidth: 1, borderColor: '#E2E8F0', padding: [12, 16] },
    ...
    const barOptionTemplate = (data: any[], colorStops: any[]) => ({
        ...
        yAxis: { type: 'category', data: data.map(d => d.name).reverse(), axisLine: { show: false }, axisTick: { show: false }, axisLabel: { fontWeight: '600', color: '#0F172A', fontSize: 12 } },
        series: [{ type: 'bar', data: data.map(d => d.value).reverse(), itemStyle: { color: new echarts.graphic.LinearGradient(0, 0, 1, 0, colorStops), borderRadius: [0, 6, 6, 0] }, barWidth: '45%', label: { show: true, position: 'right', fontWeight: '700', fontSize: 12 } }]
    });
        ...
        <div className="pb-10 max-w-7xl mx-auto space-y-8 relative">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 animate-in fade-in">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                    <div className="relative w-full sm:w-72 z-[60]">
                        ...
                        <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="w-full flex items-center justify-between bg-white border border-slate-200 rounded-[40px] px-5 py-3 text-sm font-bold shadow-sm z-50 relative"><div className="flex items-center gap-2 truncate"><MapPin size={16} className="text-[#4f46e5]" /><span>{selectedProvince}</span></div><ChevronDown size={16} /></button>
                        {isDropdownOpen && (
                            <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-white border border-slate-100 rounded-[24px] shadow-xl z-50 overflow-hidden">
                                ...
                                <div className="max-h-[260px] overflow-y-auto p-2">
                                    ...
                                    {filteredProvinces.map(prov => <button key={prov} onClick={() => triggerDataUpdate(prov)} className="w-full text-left px-4 py-3 rounded-[16px] text-sm font-bold hover:bg-slate-50 text-slate-600">{prov}</button>)}
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div className="lg:col-span-3 bg-white border border-slate-100 rounded-[40px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] relative flex flex-col p-4 min-h-[500px]">
                    ...
                    <div className="w-full h-full min-h-[500px] bg-[#F8FAFC] rounded-[32px] overflow-hidden"><ReactECharts ref={echartsRef} option={mainMapOption} style={{ height: '100%', width: '100%' }} onEvents={{ 'click': (p: any) => p.name && triggerDataUpdate(p.name) }} /></div>
                </div>
                ...
                <div className="lg:col-span-2 bg-white border border-slate-100 rounded-[40px] p-6 lg:p-8 flex flex-col relative overflow-hidden">
                    ...
                    <div className="bg-slate-50 p-5 rounded-[24px] flex justify-between items-center mb-6">
                        <div><p className="text-[11px] font-bold text-slate-500 uppercase">Total Volume Wilayah</p><h3 className="text-2xl font-black">{mapAnalytics?.totalVolume?.toLocaleString('id-ID') || 0} Unit</h3></div>
                    ...
                    <div className="flex-1 w-full relative min-h-[250px]">
                        ...
                        {activeTab === 'produk' && <ReactECharts option={{ tooltip: { trigger: 'item' }, series: [{ type: 'pie', radius: ['45%', '70%'], data: mapAnalytics?.topProducts || [] }] }} style={{ height: '100%', width: '100%' }} />}
                    </div>
                </div>
            </div>
        </div>
