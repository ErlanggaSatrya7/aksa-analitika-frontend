...
export default function RegionalForecastPage() {
    ...
    const [currentInsightText, setCurrentInsightText] = useState('Komparasi API Logistik Provinsi vs Algoritma Random Forest: Kuota suplai agregat di Sumatera Utara bulan ini tidak akan mencukupi lonjakan permintaan sektor "Men\'s Apparel" pada minggu ke-3.');

    const [isHistoryOpen, setIsHistoryOpen] = useState(false);

    const [broadcastHistory, setBroadcastHistory] = useState([
        { id: 'ESC-REG-08', date: 'Senin, 11 Mei 2026 08:30 WIB', target: 'Pusat (Super Admin)', insight: 'Pengajuan penambahan kuota suplai Sumatera Utara untuk Women\'s Street sebesar 15.000 Pcs.' }
    ]);
    ...
    const handleGenerateForecast = () => {
        ...
        setTimeout(() => {
            ...
            setInsightTime(updateTimestamp());
            const areaText = filterCity === 'Semua Kota' ? 'seluruh kota di Sumatera Utara' : `wilayah ${filterCity}`;
    ...
    const handleBroadcastAlert = () => {
        ...
        setTimeout(() => {
            ...
            const targetPusat = 'Pusat (Super Admin)';
            const newLog = { id: `ESC-REG-${Math.floor(Math.random() * 90) + 10}`, date: updateTimestamp(), target: targetPusat, insight: currentInsightText };
    ...
    const forecastOption = {
            ...
            formatter: function (params: any) {
                ...
                sortedParams.forEach((param: any) => {
                    if (param.value !== null && param.value !== undefined) {
                        const fullValue = (param.value * 1000).toLocaleString('id-ID');
        ...
        series: [
            // UPDATE: Warna selaras gradasi premium
            { name: 'Kapasitas Gudang Provinsi', type: 'line', smooth: true, symbolSize: 8, itemStyle: { color: '#4f46e5', borderWidth: 2, borderColor: '#fff' }, lineStyle: { width: 4, color: '#4f46e5' }, areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(106, 123, 250, 0.4)' }, { offset: 1, color: 'rgba(79, 70, 229, 0)' }] } }, data: [45, 52, 48, 65, 78, null, null, null] },
            { name: 'Skenario Optimis', type: 'line', smooth: true, symbolSize: 6, itemStyle: { color: '#10B981', borderWidth: 2, borderColor: '#fff' }, lineStyle: { width: 2, type: 'dashed', color: '#10B981' }, data: [null, null, null, null, 78, 85, 98, 115] },
            {
                name: 'Prediksi Utama', type: 'line', smooth: true, symbolSize: 8, itemStyle: { color: '#F59E0B', borderWidth: 2, borderColor: '#fff' }, lineStyle: { width: 3, type: 'dashed', color: '#F59E0B' }, data: [null, null, null, null, 78, 82, 89, 95],
            ...
            { name: 'Skenario Pesimis', type: 'line', smooth: true, symbolSize: 6, itemStyle: { color: '#EF4444', borderWidth: 2, borderColor: '#fff' }, lineStyle: { width: 2, type: 'dashed', color: '#EF4444' }, data: [null, null, null, null, 78, 79, 81, 83] }
        ]
    };

    if (isLoading) return <div className="w-full h-full flex items-center justify-center text-[#6A7BFA] gap-3"><Loader2 size={24} className="animate-spin" /><span className="font-bold tracking-widest uppercase text-sm">Memuat AI Engine...</span></div>;
        ...
        <div className="pb-10 max-w-7xl mx-auto space-y-6 relative">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1 flex items-center gap-1.5"><Info size={16} className="text-[#6A7BFA]" /> Analisis tren gabungan Sumatera Utara & eskalasi kuota ke Pusat.</p>
                </div>
            </div>
                    ...
                    <div ref={filterRef} className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div className={`flex flex-col gap-1.5 relative transition-all ${openDropdown === 'city' ? 'z-50' : 'z-10'}`}>
                            ...
                            <button onClick={() => setOpenDropdown(openDropdown === 'city' ? null : 'city')} className={`flex items-center justify-between bg-white border ${openDropdown === 'city' ? 'border-[#4f46e5] ring-4 ring-[#6A7BFA]/10 shadow-sm' : 'border-slate-200 hover:border-[#6A7BFA]/50 hover:shadow-md hover:-translate-y-0.5'} rounded-[20px] px-4 py-3.5 text-sm font-bold text-slate-700 transition-all duration-300 group`}>
                                <div className="flex items-center gap-2 truncate"><Building2 size={16} className="text-[#6A7BFA] group-hover:text-[#4f46e5] group-hover:scale-110 transition-all" /><span className="truncate group-hover:text-[#4f46e5] transition-colors">{filterCity}</span></div>
                                <ChevronDown size={16} className={`text-slate-400 group-hover:text-[#4f46e5] transition-transform ${openDropdown === 'city' ? 'rotate-180' : ''}`} />
                            </button>
                            {openDropdown === 'city' && (
                                <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-white border border-slate-100 rounded-[20px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] z-[60] p-2 animate-in fade-in slide-in-from-top-2">
                                    {['Semua Kota', 'Medan', 'Binjai', 'Pematangsiantar', 'Tebing Tinggi'].map((item) => (
                                        <button key={item} onClick={() => { setFilterCity(item); setOpenDropdown(null); }} className={`w-full flex items-center justify-between px-4 py-2.5 rounded-[12px] text-sm font-bold transition-all duration-200 ${filterCity === item ? 'bg-gradient-to-r from-[#6A7BFA] to-[#4f46e5] text-white' : 'text-slate-600 hover:bg-[#F4F7FE] hover:text-[#4f46e5]'}`}>
                        ...
                        <div className={`flex flex-col gap-1.5 relative transition-all ${openDropdown === 'retailer' ? 'z-50' : 'z-10'}`}>
                            ...
                            <button onClick={() => setOpenDropdown(openDropdown === 'retailer' ? null : 'retailer')} className={`flex items-center justify-between bg-white border ${openDropdown === 'retailer' ? 'border-[#4f46e5] ring-4 ring-[#6A7BFA]/10 shadow-sm' : 'border-slate-200 hover:border-[#6A7BFA]/50 hover:shadow-md hover:-translate-y-0.5'} rounded-[20px] px-4 py-3.5 text-sm font-bold text-slate-700 transition-all duration-300 group`}>
                                <div className="flex items-center gap-2 truncate"><Store size={16} className="text-[#6A7BFA] group-hover:text-[#4f46e5] group-hover:scale-110 transition-all" /><span className="truncate group-hover:text-[#4f46e5] transition-colors">{filterRetailer}</span></div>
                        ...
                        <div className={`flex flex-col gap-1.5 relative transition-all ${openDropdown === 'product' ? 'z-50' : 'z-10'}`}>
                            ...
                            <button onClick={() => setOpenDropdown(openDropdown === 'product' ? null : 'product')} className={`flex items-center justify-between bg-white border ${openDropdown === 'product' ? 'border-[#4f46e5] ring-4 ring-[#6A7BFA]/10 shadow-sm' : 'border-slate-200 hover:border-[#6A7BFA]/50 hover:shadow-md hover:-translate-y-0.5'} rounded-[20px] px-4 py-3.5 text-sm font-bold text-slate-700 transition-all duration-300 group`}>
                                <div className="flex items-center gap-2 truncate"><PackageSearch size={16} className="text-[#6A7BFA] group-hover:text-[#4f46e5] group-hover:scale-110 transition-all" /><span className="truncate group-hover:text-[#4f46e5] transition-colors">{filterProduct}</span></div>
            ...
            <div className="bg-white border border-slate-100 rounded-[40px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] p-6 md:p-8 flex flex-col relative overflow-hidden animate-in fade-in slide-in-from-bottom-6 duration-700 ease-out delay-200 fill-mode-both z-10">
                {isPredicting && (
                    <div className="absolute inset-0 bg-white/90 backdrop-blur-md z-20 flex flex-col items-center justify-center text-[#4f46e5] gap-4 animate-in fade-in duration-300">
                        ...
                        <span className="font-bold text-xl text-slate-800">Menyinkronkan API Gudang Provinsi...</span>
                    </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
                    <div className="flex items-center gap-2 bg-[#EDF2FE] px-4 py-2 rounded-full text-[#4f46e5] text-xs font-bold border border-[#6A7BFA]/20 w-fit">
                        <BrainCircuit size={16} /> Regional Macro Prediction Model
                    </div>
                    <div className="flex items-center gap-2 bg-emerald-50 px-4 py-2 rounded-full text-emerald-600 text-xs font-bold border border-emerald-100 w-fit shadow-sm">
                        ...
                        API Gudang Provinsi Tersinkronisasi
                    </div>
                </div>
                    ...
                    <div className="mt-8 p-6 lg:p-8 border bg-[#F8FAFC] border-slate-200 rounded-[32px] shadow-inner relative overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6 relative z-10 border-b border-slate-200 pb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h4 className="font-bold text-xl text-slate-900 flex items-center gap-2">Tindakan Eksekutif <span className="px-2 py-0.5 bg-amber-500 text-white text-[10px] uppercase tracking-widest rounded-md animate-pulse shadow-sm">Urgent</span></h4>
                                    <p className="text-sm font-medium text-slate-500 mt-0.5">Eskalasi permohonan kuota ke Pusat Nasional.</p>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 relative z-10 mb-6">
                            <div className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex flex-col gap-3 hover:shadow-md transition-shadow">
                                <div className="flex items-center gap-2 text-slate-800"><Info size={18} className="text-[#4f46e5]" /><span className="font-bold text-xs uppercase tracking-widest">Kondisi Sumatera Utara</span></div>
                            ...
                            <div className="bg-red-50 p-5 rounded-[24px] border border-red-100 shadow-sm flex flex-col gap-3 hover:shadow-md transition-shadow">
                                <div className="flex items-center gap-2 text-red-600"><TrendingDown size={18} /><span className="font-bold text-xs uppercase tracking-widest">Risiko Defisit Provinsi</span></div>
                                <p className="text-red-900/80 text-sm font-medium leading-relaxed">Tanpa tambahan kuota pengiriman dari gudang pusat, <strong className="text-red-700">seluruh kota di Sumatera Utara</strong> akan mengalami hambatan distribusi dan gagal mencapai target Kuartal Q2.</p>
                            </div>
                        </div>
                ...
                <button onClick={() => setIsHistoryOpen(!isHistoryOpen)} className="flex items-center justify-between w-full group">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="text-left">
                            <h3 className="text-xl font-bold text-slate-900">Riwayat Eskalasi Nasional</h3>
                            <p className="text-sm text-slate-500 mt-0.5">Catatan permohonan kuota yang telah diajukan ke Super Admin.</p>
                        </div>
                    </div>
