...
import { Inbox, CheckCircle2, XCircle, Send, PackagePlus, AlertTriangle, Clock, Check, Plus, PackageCheck } from 'lucide-react';
...
export default function RegionalInboxPage() {
    ...
    const [inboxItems, setInboxItems] = useState([
        {
            id: 1,
            sender: "Citra Lestari (Manager Medan)",
            title: "Pengajuan Restock: Sneakers Vans",
            desc: "AI memprediksi stok Sneakers Vans di Ramayana Plaza akan habis dalam 3 hari akibat tren akhir pekan. Mengajukan transfer 250 pasang dari gudang provinsi.",
        ...
        {
            id: 2,
            sender: "Eko Saputra (Manager Binjai)",
            title: "Laporan Kritis: Toko Sepi",
            desc: "Terjadi penurunan drastis pengunjung di cabang Binjai Supermall. Mengusulkan agar diizinkan memasang diskon Flash Sale 30% untuk produk baju anak.",
    ...
    const handleApprove = (id: number) => {
        ...
        showToast("Permintaan cabang disetujui. Tim logistik akan segera memproses.", "success");
    };

    const handleReject = (id: number) => {
        setInboxItems(items => items.map(item => item.id === id ? { ...item, status: 'rejected' } : item));
        showToast("Permintaan ditolak. Silakan berikan alasan ke manajer kota terkait.", "error");
    };
        ...
        <div className="pb-10 max-w-6xl mx-auto animate-in fade-in duration-500 relative">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-xs md:text-sm text-slate-500 mt-1">Evaluasi permohonan cabang dan teruskan instruksi ke Super Admin.</p>
                </div>
                {/* UPDATE: Tombol Buat Memo */}
                ...
                <div className="flex border-b border-slate-100 px-6 md:px-8 pt-6 gap-6">
                    <button onClick={() => setActiveTab('masuk')} className={`pb-4 text-sm font-bold transition-all border-b-2 ${activeTab === 'masuk' ? 'border-[#4f46e5] text-[#4f46e5]' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>
                        <div className="flex items-center gap-2">
                            <Inbox size={18} /> Kotak Masuk Kota
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                                        ...
                                        {item.status === 'pending' && (
                                            <div className="flex items-center gap-2 mt-4 md:mt-0 pt-4 md:pt-0 border-t border-slate-100 md:border-none shrink-0 w-full md:w-auto">
                                                ...
                                                {/* UPDATE: Tombol Izinkan Premium */}
                                                <button onClick={() => handleApprove(item.id)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#6A7BFA] to-[#4f46e5] text-white font-bold text-xs hover:shadow-md transition-all shadow-sm active:scale-95">
                        ...
                        <div className="flex flex-col items-center justify-center h-full min-h-[350px] animate-in fade-in slide-in-from-bottom-4">
                            <div className="relative mb-6">
                                ...
                                <div className="w-24 h-24 bg-white border border-slate-100 shadow-xl rounded-full flex items-center justify-center relative z-10 text-[#4f46e5]">
                                    <PackageCheck size={40} className="animate-bounce" />
                                </div>
                            </div>
                            <h4 className="text-xl font-bold text-slate-800 mb-2">Pusat Permohonan Bersih!</h4>
                            <p className="text-sm text-slate-500 max-w-sm text-center leading-relaxed">
                                Anda belum mengirimkan permohonan suplai barang atau memo apapun ke pusat (Super Admin) hari ini.
                            </p>
                        </div>
                    )}
                </div>
