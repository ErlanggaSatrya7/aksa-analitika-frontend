...
import { Wallet, Package, Percent, Activity, LayoutList, ShoppingBag, Map as MapIcon, Loader2, ArrowRight, PackageSearch, Store, BarChart3, Globe } from 'lucide-react';
...
export default function RegionalDashboardPage() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            fetch(`/api/regional/analytics/dashboard?state=${encodeURIComponent(userState)}`).then(res => res.json())
        ])
            .then(([mapData, dbData]) => {
                if (mapData && mapData.features) {
                    // 🔥 KAMUS PENERJEMAH GEOJSON KE DATABASE
                    ...
                    // Saring Peta HANYA untuk Provinsi User yang sedang login
                    mapData.features = mapData.features.filter((feature: any) => {
                        ...
                        // 🔥 KUNCI UTAMA: Tanamkan "name" agar ECharts bisa membacanya!
                ...
                setAvailableCities(dbData.cities || []);
                setIsLoading(false);
            })
            .catch((err) => {
    ...
    // 🔥 PERBAIKAN PETA: Zoom Limit, Roam, dan Pemanggilan nameProperty
    const mapOption = useMemo(() => {
        const mapData = dashboardData?.mapDistribution || [{ name: userState.toUpperCase(), value: 0 }];
        return {
            ...
            tooltip: {
                ...
                formatter: (params: any) => `
                    ...
                    <div style="color:#6A7BFA; font-weight:bold; font-size:15px;">${params.name || 'Memuat...'}</div>
                    <div style="color:#0F172A; font-weight:bold; margin-top:4px;">Total Volume: ${(params.value || 0).toLocaleString('id-ID')} Unit</div>
                    <div style="font-size:10px; color:#10B981; margin-top:6px; background:#ECFDF5; padding:2px 6px; border-radius:4px; display:inline-block; font-weight:bold;">● Status: Aktif</div>
                `
            },
            visualMap: { min: 0, max: 50000, show: false, inRange: { color: ['#EDF2FE', '#A3B1FF', '#4f46e5'] } },
            series: [{
                type: 'map',
                map: 'regional_view',
                nameProperty: 'name', // Memastikan ECharts membaca properties.name
                roam: true,           // 🔥 AKTIFKAN Fitur Geser & Zoom
                scaleLimit: { min: 0.5, max: 10 }, // 🔥 BATAS ZOOM IN & OUT (10x lipat)
                zoom: 1.2,
    ...
    const trendChartOption = {
        ...
        xAxis: { type: 'category', boundaryGap: false, data: dashboardData?.trendLine?.labels || [], axisLine: { lineStyle: { color: '#E2E8F0' } } },
    ...
    const activeProducts = selectedCity === 'Semua Kota'
        ? dashboardData?.topProducts || []
        : dashboardData?.cityProducts?.[selectedCity] || [];
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white rounded-[32px] p-6 lg:p-8 border border-slate-100 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 shadow-[0_8px_30px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_40px_-15px_rgba(79,70,229,0.2)] group cursor-default">
                    ...
                    <div>
                        ...
                        <h3 className="text-3xl lg:text-4xl font-bold text-slate-900 tracking-tight">Rp {(dashboardData?.summary?.totalSales / 1000000000).toFixed(1) || 0} <span className="text-xl lg:text-2xl text-slate-400 font-medium tracking-normal">Miliar</span></h3>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[32px] p-6 lg:p-8 border border-slate-100 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 shadow-[0_8px_30px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_40px_-15px_rgba(79,70,229,0.2)] group cursor-default">
                    ...
                    <div>
                        <p className="text-[11px] lg:text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5 group-hover:text-[#4f46e5] transition-colors">Persentase Margin</p>
                ...
                <div className="bg-white rounded-[32px] p-6 lg:p-8 border border-slate-100 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 shadow-[0_8px_30px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_40px_-15px_rgba(79,70,229,0.2)] group cursor-default">
                    ...
                    <div>
                        <p className="text-[11px] lg:text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5 group-hover:text-[#4f46e5] transition-colors">Total Volume Barang</p>
                        <h3 className="text-3xl lg:text-4xl font-bold text-slate-900 tracking-tight">{(dashboardData?.summary?.totalUnits / 1000).toFixed(1) || 0}k <span className="text-xl lg:text-2xl text-slate-400 font-medium tracking-normal">Pcs</span></h3>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div className="lg:col-span-2 bg-white border border-slate-100 rounded-[40px] p-6 lg:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.03)] flex flex-col group relative overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out delay-200 fill-mode-both">
                    ...
                    <div className="flex flex-wrap gap-2 mb-8 bg-slate-50 p-1.5 rounded-[20px] border border-slate-100 w-fit">
                        {['Semua Kota', ...availableCities].slice(0, 4).map((city) => (
                            <button key={city} onClick={() => handleCityClick(city)}
                                className={`px-4 py-2 rounded-[16px] text-xs font-bold transition-all ${selectedCity === city ? 'bg-gradient-to-r from-[#6A7BFA] to-[#4f46e5] text-white shadow-md border-transparent' : 'text-slate-500 hover:text-slate-700 hover:bg-white/50 border border-transparent'}`}>
                                {city}
                            </button>
                        ))}
                    </div>
