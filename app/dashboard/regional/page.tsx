...
export default function RegionalDashboardPage() {
    ...
    // BLOK: SISTEM DIAGNOSTIK
    // Mengecek latensi dan status FastAPI
    ...
    // BLOK: STATE COMMAND CENTER FEED
    // Menyimpan instruksi dari eksekutif
    const [instructions, setInstructions] = useState<any[]>([]);

    // BLOK: FETCH DATA DASHBOARD
    // Memuat geojson, analitik, dan notifikasi
    useEffect(() => {
        ...
        Promise.all([
            ...
            .then(async ([mapData, resDb, notifs]) => {
                ...
                // BLOK: LOGIKA PETA
                // Filter GeoJSON hanya untuk provinsi user yang login
                ...
                // BLOK: FILTER MEMO TARGET
                // Mengambil 3 instruksi Command Center terbaru
    ...
    // BLOK: OPSI PETA WARNA INDIGO PREMIUM
    const mapOption = useMemo(() => {
        ...
        return {
            ...
            visualMap: { min: 0, max: 100000, show: false, inRange: { color: ['#EDF2FE', '#6A7BFA'] } },
            series: [{
                type: 'map',
                map: 'regional_view',
                nameProperty: 'name',
                ...
                itemStyle: { areaColor: '#EDF2FE', borderColor: '#4f46e5', borderWidth: 1.5 },
    ...
    // BLOK: OPSI LINE CHART (TREN PERFORMA)
    ...
    // BLOK: OPSI BAR CHART (METODE PENJUALAN)
    ...
    // BLOK: OPSI BAR CHART (TOP PRODUK)
    ...
    // BLOK: FUNGSI MAPPER NAMA RETAILER
    // Mengonversi ID Database angka menjadi string nama yang rapi
    const mappedRetailerData = useMemo(() => {
        if (!dashboardData?.retailerShare) return [];

        // Silakan isi string ini dengan nama sebenarnya dari database Admin
        const retailerNames: Record<string, string> = {
            "1000001": "Nama Retailer 1",
            "1000002": "Nama Retailer 2",
            "1000003": "Nama Retailer 3",
            "1000004": "Nama Retailer 4",
            "1000005": "Nama Retailer 5",
            "1000006": "Nama Retailer 6"
        };

        return dashboardData.retailerShare.map((r: any) => {
            const idString = r.name?.toString();
            // Jika ID ada di kamus, gunakan namanya. Jika tidak, tetap tampilkan angkanya
            const finalName = retailerNames[idString] || idString;
            return { ...r, name: finalName };
        });
    }, [dashboardData]);

    // BLOK: OPSI PIE CHART (PORTOFOLIO RETAILER)
    const retailerChartOption = {
        ...
        series: [{ name: 'Retailer', type: 'pie', radius: ['40%', '70%'], center: ['50%', '45%'], avoidLabelOverlap: false, itemStyle: { borderRadius: 8, borderColor: '#fff', borderWidth: 3 }, label: { show: false }, data: mappedRetailerData }]
    };
    ...
    return (
        <div className="pb-10 max-w-7xl mx-auto space-y-8">
            ...
            {/* BLOK: KARTU METRIK & STATUS AI ENGINE */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div className="lg:col-span-3 bg-white rounded-[40px] p-6 lg:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.03)] border border-slate-100 flex flex-col relative overflow-hidden group">
                    ...
                    <div className="w-full h-[320px] lg:h-[380px] relative z-10 rounded-[24px] bg-[#F8FAFC] border border-slate-100 overflow-hidden group-hover:shadow-inner transition-shadow duration-500">
                        {isMapLoaded ? (
                ...
                <div className="lg:col-span-2 bg-white border border-slate-100 rounded-[40px] p-6 lg:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.03)] flex flex-col group relative overflow-hidden">
                    <div className="mb-4">
                        ...
                        <p className="text-xs font-medium text-slate-500">Berdasarkan volume penjualan aktual bulan ini di {userState}.</p>
                    </div>

                    <div className="space-y-6 flex-1 flex flex-col overflow-y-auto custom-scrollbar pr-2 pt-4 pb-2">
            ...
            {/* BLOK: BAGIAN CHART KINERJA & PORTOFOLIO */}
